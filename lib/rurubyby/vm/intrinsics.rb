  ...
  module Vm
    module Intrinsics
      ...
      class << self
        ...
        # Integer
        # native integers
        def is_int(v) = v.class.equal?(Integer)
        ...
        def def_int_cmp   (name, op) = eval "def int_#{name}(v1, v2); #{check_int_bin_args(op)}; bool_object_for(v1 #{op} v2); end"
        #def def_int_cmp   (name, op) = eval "def int_#{name}(v1, v2); #{check_int_bin_args(op)}; bool_object_for(v1.value #{op} v2.value); end"

        def def_int_bin_op(name, op) = eval "def int_#{name}(v1, v2); #{check_int_bin_args(op)}; v1 #{op} v2; end"
