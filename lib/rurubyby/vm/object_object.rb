  ...
  module Vm
    class ObjectObject
      ...
      class << self
        ...
        def rurubyby___id__(v) = v.__id__

        def rurubyby__not_(v) = (v.equal?(FalseObject::FALSE_OBJECT) || v.equal?(NilObject::NIL_OBJECT)) ? TrueObject::TRUE_OBJECT : FalseObject::FALSE_OBJECT

        def rurubyby_equal?(v1, v2) = v1.__id__.equal?(v2.__id__)

        def rurubyby__equals_equals_(v1, v2) = v1.__id__.equal?(v2.__id__)
      end
    end
  end
end
